...
class ZipBackupWriter(
    ...
    private fun ZipOutputStream.putArchive(contents: BackupContents) {
        ...
        val photoFiles = contents.encounters.flatMap { it.photos }.flatMap { listOfNotNull(it.photoPath, it.thumbPath) }
        photoFiles.distinct().forEach { path ->
    ...
    private fun readArchive(source: String, staging: File): BackupReadResult {
        ...
        return when {
            ...
            manifest.formatVersion >= FIRST_FORMAT_WITH_WALKS && !unpacked.texts.keys.containsAll(archiveTextEntries) ->
                ...
                contents.encounters.flatMap { it.photos }.forEach { photo ->
                    listOfNotNull(photo.photoPath, photo.thumbPath).forEach(photoStorage::fileFor)
                }
