...
it has (`EncounterDaoAttachPhotoTest`). `update`, which a backup's later edit uses, rewrites a row but keeps the photo columns it has, and a
backup's photos are restored only onto a row without one, leaving its `updatedAt` alone
(`EncounterDaoRestorePhotoTest`). `setCoat` writes only the `coat` column and `updatedAt`,
