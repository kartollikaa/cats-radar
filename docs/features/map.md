...
`INTERNET` permission at all: geocoding runs through the platform. Crash reports go to Firebase as
well (`analytics.md`), but no screen waits on them. Tiles are fetched as the map is looked at,
