...
the finger lifts. On the way the phone ticks softly each time the fill crosses the next of
`TicksPerHold` even steps, so the hold can be felt working without looking at it; a press that picks
the fill up mid-drain ticks on from where it is rather than from the start. Let go earlier, or drift
off the button, and the ticks stop, the fill drains back and nothing changes. The button's second
line says *press and hold* meanwhile, after the walk's time: a gesture nothing hints at is one nobody
finds.
