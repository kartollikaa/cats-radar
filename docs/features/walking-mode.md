...
- **The walk ending is not news to the notification.** **Done** turns the mode off and ends the
  ...
  was swiped away.
- **The button's cat walks only while a walk is on.** With no walk it stands on its first frame,
  legs straight down. The notification's icon never moves: it is the cat's face, not the walking
  cat (see [A Live Update, from API 36.1](#a-live-update-from-api-361)).
...
- `ui/…/res/drawable/cat_walk_*.xml` — the button's cat, generated by `tools/make-walking-cat.py`;
  `ui/…/counter/WalkingCat.kt` — the button's loop
