...
- **A sheet is a `CatsRadarBottomSheet`**, never Material's `ModalBottomSheet`, which stops a tall
  sheet half open: the app's sheets open all the way (`BottomSheetUsageTest`).
