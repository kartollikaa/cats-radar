...
## What R8 does to a release build
...
- **A release stack trace is unreadable without its mapping.** Every build writes
  ...
  attached to the GitHub release next to that APK. `retrace mapping.txt stacktrace.txt` (the `retrace`
  tool from the SDK's Command-line Tools) turns the short names back.
...
5. `gh release create v<versionName> --prerelease --target <merge commit>`, with the APK attached as
   `cats-radar-<versionName>.apk` and its `mapping.txt` as `cats-radar-<versionName>-mapping.txt`.
