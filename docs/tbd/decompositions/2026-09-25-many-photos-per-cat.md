...
| M8 | Several photos from the gallery at once | *Choose from gallery* picks several images and attaches them one after another, with progress and one message for the lot. | safe | ~450 | M7 | merged |
...
- 2026-09-26: **M8 merged** as #177; **the epic is complete**. A pick carries on after its cat is removed,
  but each attempt reads the cat first, so the rest fail at once; the count lives in the bar's fill and its
  spoken description rather than in visible text.
