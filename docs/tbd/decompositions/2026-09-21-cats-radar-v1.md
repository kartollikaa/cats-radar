...
- 2026-09-24: **#88's location request did not reach real phones**; imports now ask for `ACCESS_MEDIA_LOCATION` and open the gallery with `ACTION_GET_CONTENT`. MediaProvider redacts every `ACTION_PICK_IMAGES` photo whatever the app holds, and `EXTRA_REQUEST_LOCATION_METADATA_ACCESS` is honoured only by photo-picker builds whose rollout flag is on — true on the API 37.1 emulator #88 was verified on, not on the owner's phone, where the picker never asked and photos kept arriving without GPS. A `GET_CONTENT` photo keeps its GPS for an app holding the permission (verified on the emulator both ways: coordinates and the file's own digest with it, `NONE` and a redacted digest without). The system words the dialog as access to photos and videos; no in-app rationale for now.
- 2026-09-24: **imported photos carry their location after all**, superseding the 2026-09-22 spike result below. The Photo Picker learned a location-metadata request (`MediaStore.EXTRA_REQUEST_LOCATION_METADATA_ACCESS`) through its own system update; with it the picker asks the user and, if they agree, hands over the original bytes with their GPS. The import now asks (#88). Still no runtime permission; cats imported before it keep no location.
...
- 2026-09-22: slice 10 renames `PhotoStore` to `PhotoStorage` — Konsist reserves the `*Store` suffix
  ...
  default list covers every other test source set; AGP's KMP plugin renamed `androidUnitTest`).
  Exempting `FunctionNaming` there was considered and dropped: it would force renaming every
  existing `androidHostTest` test, which is mechanical churn that does not belong in a feature slice.
