...
|---|----------|------------------------|----------|-------------|------------|--------|
| P1a | Attaching a photo, in domain and data | `AttachPhoto` and a guarded write that touches only the photo columns of a live cat with no photo; "With photo" counted by the photo itself. Nothing calls it yet. | safe | ~450 | v1 | in-progress |
